...
function renderMessages(s){if(current!==s)return;const list=Array.from(s.messages.values()).sort((a,b)=>a.ts-b.ts||a.id.localeCompare(b.id)),nodes=[],reactions=new Map();let previous=null,lastDay='';
 ...
 for(const m of list){if(m.type==='reaction'||m.type==='other'&&/protocol|album|senderKey/i.test(m.kind||''))continue;
  ...
  let node=s.nodes.get(m.id);const signature=JSON.stringify([m.type,m.text,m.template,m.contacts,m.quote,m.fileName,m.transcript,m.transcription,m.localStatus]);
...
function bubble(m,s){const d=document.createElement('article');d.className='m'+(m.template?' commercial':'')+(m.fromMe?' me':'')+(m.type==='audio'?' audio':'')+(['image','sticker','video'].includes(m.type)?' photo':'')+(m.localStatus?.includes('confirmado')?' failed':'');d.dataset.id=m.id;
 ...
 else if(m.type==='text')html+=`<div class="body-text">${linkify(m.text)}</div>`;
