...
let localOnly=false,resuming=false,activeScope='';
...
async function api(path,options={}){
 if(localOnly&&path!=='/api/session'&&path!=='/api/login'&&path!=='/api/logout')throw new Error('Modo offline: conecte-se para sincronizar ou enviar. Seu rascunho fica salvo.');
 ...
 if(!response.ok){const e=new Error(typeof data.detail==='string'?data.detail:'Não foi possível concluir. Tente novamente.');e.status=response.status;e.uncertain=response.status>=500;
  if(response.status===401&&path!=='/api/login'){window.waOffline?.revoke();if(session)showLogin('Sua sessão expirou. Entre para continuar; seus rascunhos foram preservados.');}throw e;}
 return data;
}
...
async function loadChat(s=current,older=false){if(!s||!session||s.loading||s.older||older&&!s.hasMore)return;
 if(localOnly){const epoch=authEpoch;const cached=await window.waCache?.get('chat:'+s.key);if(epoch!==authEpoch)return;if(cached?.messages){mergeMessages(s,cached.messages);s.initialized=true;s.cached=true;if(current===s)renderMessages(s);}else if(current===s)chatError('Esta conversa ainda não foi salva neste aparelho. Conecte-se para carregá-la.');return;}
...
function renderMessages(s){if(current!==s)return;const list=Array.from(s.messages.values()).sort(compareMessages),nodes=[],reactions=new Map();let previous=null,lastDay='';
 ...
 for(const m of list){if(m.type==='reaction'||m.type==='other'&&/protocol|album|senderKey/i.test(m.kind||''))continue;
  ...
  let node=s.nodes.get(m.id);const signature=JSON.stringify([m.type,m.text,m.displayText,m.template,m.contacts,m.quote,m.fileName,m.transcript,m.transcription,m.localStatus]);
...
function templateText(text){return String(text||'').split(/(\*[^*\n]+\*)/g).map(part=>part.startsWith('*')&&part.endsWith('*')?`<strong>${esc(part.slice(1,-1))}</strong>`:linkify(part)).join('');}
function templateCard(t){return `<section class="template-card">${t.title?`<div class="template-title">${templateText(t.title)}</div>`:''}<div class="body-text">${templateText(t.body)}</div>${t.footer?`<div class="template-footer">${esc(t.footer)}</div>`:''}${(t.actions||[]).length?`<div class="template-actions">${t.actions.map(a=>/^https?:\/\//i.test(a.url||'')?`<a href="${esc(a.url)}" target="_blank" rel="noopener noreferrer"><span aria-hidden="true">↗</span> ${esc(a.label)}</a>`:`<span class="template-option">${esc(a.label)}<small>Responda pela conversa</small></span>`).join('')}</div>`:''}</section>`;}
function bubble(m,s){const d=document.createElement('article');d.className='m'+(m.template?' commercial':'')+(m.fromMe?' me':'')+(m.type==='audio'?' audio':'')+(['image','sticker','video'].includes(m.type)?' photo':'')+(m.localStatus?.includes('confirmado')?' failed':'');d.dataset.id=m.id;
 ...
 if(m.template)html+=templateCard(m.template);
 else if(m.type==='text')html+=`<div class="body-text">${linkify(m.displayText??m.text)}</div>`;
 ...
 else if(m.type==='image'||m.type==='sticker')html+=`<button type="button" class="media-open" aria-label="Abrir imagem"><img src="${esc(src)}" alt="${esc(m.text||'Imagem recebida')}" loading="lazy" width="320" height="220"></button><div class="body-text">${linkify(m.displayText??m.text)}</div>`;
 else if(m.type==='video')html+=`<button type="button" class="video-open" aria-label="Abrir vídeo"><video muted playsinline preload="metadata" src="${esc(src)}" aria-hidden="true"></video><span class="video-play">▶</span><span class="video-label">Vídeo · Abrir</span></button><div class="body-text">${linkify(m.displayText??m.text)}</div>`;
 ...
 else if(m.type==='document')html+=`<a class="document" href="${esc(src)}" download="${esc(m.fileName||'documento')}">${icon('file')}<span>${esc(m.fileName||'Documento')}<small>${esc(m.mime||'Arquivo')}${Number(m.size)?' · '+Math.ceil(Number(m.size)/1024)+' KB':''}</small></span></a><div class="body-text">${linkify(m.displayText??m.text)}</div>`;
 else html+=`<div class="body-text muted">${esc(m.text||'Mensagem não disponível neste visualizador.')}</div>`;
...
async function start(info,offline=false){
...
async function reconnectOffline(){
 if(resuming||!localOnly)return;resuming=true;const epoch=authEpoch;
 try{const info=await api('/api/session');if(!session||epoch!==authEpoch)return;if(info.cacheScope!==activeScope){await clearLocalSession();await start(info);return;}localOnly=false;window.waOffline.remember(info);canTranscribe=Boolean(info.transcriber);syncError='';loadList();loadChat();checkState();resizeComposer();}
 catch(e){if(e.status===401)await clearLocalSession();}
 finally{resuming=false;}
}
window.addEventListener('online',reconnectOffline);
setInterval(()=>{if(localOnly){if(!window.waOffline.get()){showLogin('Acesso offline expirou. Conecte-se para entrar novamente.');return;}if(navigator.onLine)reconnectOffline();}},15000);
async function boot(){
 try{if(localStorage.getItem('wa-logout-pending')){await post('/api/logout',{});localStorage.removeItem('wa-logout-pending');}const info=await api('/api/session',{timeout:4000});await start(info);}
