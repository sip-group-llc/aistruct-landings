...
function showLogin(message=''){session=false;authEpoch++;clearInterval(listTimer);clearInterval(chatTimer);clearInterval(stateTimer);$('#app').hidden=true;$('#login').hidden=false;$('#login-error').hidden=!message;$('#login-error').textContent=message;document.querySelectorAll('dialog[open]').forEach(d=>d.close());}
...
function saveCurrent(){if(!current)return;current.scroll=$('#msgs').scrollTop;current.atBottom=atBottom();drafts[current.key]=$('#txt').value;persistDrafts();current.visible.clear();readObserver.disconnect();}
...
function renderMessages(s){if(current!==s)return;const list=Array.from(s.messages.values()).sort((a,b)=>a.ts-b.ts||a.id.localeCompare(b.id)),nodes=[],reactions=new Map();let previous=null,lastDay='';
 ...
 for(const m of list){if(m.type==='reaction'||m.type==='other'&&/protocol|album|senderKey/i.test(m.kind||''))continue;
  ...
  const receipt=node.querySelector('.receipt');if(receipt)receipt.textContent=m.fromMe?msgStatus(m):'';
...
function bubble(m,s){const d=document.createElement('article');d.className='m'+(m.fromMe?' me':'')+(m.type==='audio'?' audio':'')+(['image','sticker','video'].includes(m.type)?' photo':'')+(m.localStatus?.includes('confirmado')?' failed':'');d.dataset.id=m.id;
 const mediaJid=m.jid||s.chat.jid,src='/api/media?'+new URLSearchParams({jid:mediaJid,id:m.id});
...
$('#txt').addEventListener('input',()=>{if(current){drafts[current.key]=$('#txt').value;persistDrafts();resizeComposer();renderList();}});
...
$('#logout').onclick=async()=>{try{await post('/api/logout',{});drafts={};persistDrafts();states.clear();listNodes.clear();chats=[];current=null;$('#list').replaceChildren();$('#messages').replaceChildren();$('#txt').value='';$('#app').classList.remove('chat-open');$('#main').hidden=true;$('#empty').hidden=false;history.replaceState({},'');showLogin();}catch(e){toast(e.message);}};
